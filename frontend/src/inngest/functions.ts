...
export const generateSong = inngest.createFunction(
  { 
    ...
    concurrency: {
    limit: 1,
    key: "event.data.userId", // Ensure that only one song is generated per user at a time
  },
    onFailure: async ({ event, error }) => {    
        await db.song.update({
            where: {id: (event?.data?.event?.data as { songId: string }).songId,},
            data: { status: "failed" },
        });
    },
},
  ...
  async ({ event, step }) => {
    ...
    if (credits>0){
        ...
        // set song status to not enough credits
