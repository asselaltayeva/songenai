    ...
    const typeNextChar = () => {
      if (isDeleting) {
        if (displayedText.length > 0) {
          ...
          frame = setTimeout(typeNextChar, initialDelay);
        }
      } else {
        if (currentCharIndex < (processedText?.length ?? 0)) {
          ...
          if (loop || currentTextIndex < textArray.length - 1) {
            frame = setTimeout(() => setIsDeleting(true), pauseDuration);
          }
        }
      }
    };
  
    frame = setTimeout(typeNextChar, currentCharIndex === 0 ? initialDelay : 0);
